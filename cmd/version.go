...
var versionCmd = &cobra.Command{
	...
	Long:  fmt.Sprintf(`Print the version information for the %v application.`, AppName),
...
var versionAppCmd = &cobra.Command{
	Use:   AppName,
	Short: "Print the version information for " + AppName,
	Long:  `Print the version information for the ` + AppName + ` application.`,
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		return nil, cobra.ShellCompDirectiveNoFileComp
	},
	Run: func(cmd *cobra.Command, args []string) {
...
var versionDockerComposeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		out, err := RunDockerComposeCommand([]string{"version", "--short"}, true)
		if err != nil {
			log.Fatalln(err)
		}
