...
func (suite *DockerComposeTestSuite) TestMockClient_Version() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			c := NewMockClient("", []byte("2.13.0"), nil)
			got, err := c.Version()
			if (err != nil) != tt.wantErr {
				t.Errorf("Version() error = %v, wantErr %v", err, tt.wantErr)
				return
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			if got := c.isMinimumVersionInstalled(); got != tt.want {
				t.Errorf("minimumVersionInstalled() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func (suite *DockerComposeTestSuite) TestClient_RunCommand() {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				args: []string{"version", "--short"},
				opts: []shell.Opt{shell.WithSuppressOutput(true)},
			},
			want:    []byte("2.13.0\n"),
			wantErr: false,
		},
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			if len(tt.args.opts) > 0 && reflect.DeepEqual(tt.args.opts[0], shell.WithSuppressOutput(true)) {
				out, _ := io.ReadAll(r)
				// The io.ReadAll() function will return an empty byte slice but the want variable is nil
				// if !reflect.DeepEqual(out, tt.want) {
				if len(out) != len(tt.want) {
					t.Errorf("ExecuteWithOptions() got = %v, want %v", out, tt.want)
				}
			}
